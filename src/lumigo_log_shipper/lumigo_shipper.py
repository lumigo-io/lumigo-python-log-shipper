...
from lumigo_log_shipper.utils.consts import STREAM_NAME, LOG_STREAM_KIIL_SWITCH, SELF_ACCOUNT_ID, TARGET_ACCOUNT_ID, ENV
...
def ship_logs(aws_event: dict) -> int:
    ...
    if len(shipper_output) > 0 and not LOG_STREAM_KIIL_SWITCH:
        ...
        if TARGET_ACCOUNT_ID != SELF_ACCOUNT_ID or ENV not in func_arn:
